...
import	{
	...
	faUniversity, faAward, faMeteor}						from	'@fortawesome/pro-solid-svg-icons'
import { truncateAddress } from 'utils';
import Link from 'next/link';

function	ItemCollection({faIcon, title, content, reward, progress, slug}) {
	return (
		<Link href={`/collections/${slug}`} passHref>
			<div className={`relative rounded-lg bg-white shadow dark:bg-dark-background-600 w-full h-full cursor-pointer group z-10 overflow-hidden transition-colors flex flex-col md:flex-row col-span-full border border-solid border-gray-100 dark:border-none group`}>
					...
					<div className={'flex flex-row justify-between'}>
						...
						<div className={'flex rounded-md dark:bg-dark-background-600'}>
							<button
								onClick={() => null}
								type={'button'}
								className={'flex flex-row items-center justify-center px-4 py-4 font-medium text-gray-400 group-hover:text-gray-900 dark:text-dark-background-100 dark:group-hover:text-white transition-colors whitespace-nowrap'}>
...
function	SectionCollections({progress}) {
	return (
		<section id={'collections'} aria-label={'collections'}>
			<div className={'grid gap-8 grid-cols-1'}>
				<ItemCollection
					...
					slug={'all'}
					progress={((progress['all']?.claimed || 0) / (progress['all']?.count || 1) * 100)} />
				<ItemCollection
					content={'Sed nec diam a ante porttitor blandit non ut ex. Maecenas commodo aliquam ultricies. Maecenas at finibus lacus. Quisque ultrices commodo metus in molestie. Proin lacinia nisl non massa pellentesque rhoncus.'}
					faIcon={faMeteor}
					title={'Meta'}
					reward={'i-am-the-meta.eth'}
					slug={'meta'}
					progress={((progress['meta']?.claimed || 0) / (progress['meta']?.count || 1) * 100)}/>
				<ItemCollection
					content={'Sed nec diam a ante porttitor blandit non ut ex. Maecenas commodo aliquam ultricies. Maecenas at finibus lacus. Quisque ultrices commodo metus in molestie. Proin lacinia nisl non massa pellentesque rhoncus.'}
					faIcon={faShapes}
					title={'Protocols'}
					...
					slug={'protocols'}
					progress={((progress['protocols']?.claimed || 0) / (progress['protocols']?.count || 1) * 100)}/>
				<ItemCollection
					content={'Sed nec diam a ante porttitor blandit non ut ex. Maecenas commodo aliquam ultricies. Maecenas at finibus lacus. Quisque ultrices commodo metus in molestie. Proin lacinia nisl non massa pellentesque rhoncus.'}
					faIcon={faCoins}
					reward={'master-of-coin.eth'}
					slug={'erc20'}
					progress={((progress['erc20']?.claimed || 0) / (progress['erc20']?.count || 1) * 100)}
					title={'ERC-20'} />
				<ItemCollection
					content={'Sed nec diam a ante porttitor blandit non ut ex. Maecenas commodo aliquam ultricies. Maecenas at finibus lacus. Quisque ultrices commodo metus in molestie. Proin lacinia nisl non massa pellentesque rhoncus.'}
					...
					slug={'airdrop'}
					progress={((progress['airdrop']?.claimed || 0) / (progress['airdrop']?.count || 1) * 100)}
					title={'Airdrops'} />
				<ItemCollection
					content={'Sed nec diam a ante porttitor blandit non ut ex. Maecenas commodo aliquam ultricies. Maecenas at finibus lacus. Quisque ultrices commodo metus in molestie. Proin lacinia nisl non massa pellentesque rhoncus.'}
					faIcon={faGasPump}
					reward={'gas-pump.eth'}
					slug={'fees'}
					progress={((progress['fees']?.claimed || 0) / (progress['fees']?.count || 1) * 100)}
					title={'Fees'} />
				<ItemCollection
					content={'Sed nec diam a ante porttitor blandit non ut ex. Maecenas commodo aliquam ultricies. Maecenas at finibus lacus. Quisque ultrices commodo metus in molestie. Proin lacinia nisl non massa pellentesque rhoncus.'}
					faIcon={faUniversity}
					...
					slug={'defi'}
					progress={((progress['defi']?.claimed || 0) / (progress['defi']?.count || 1) * 100)}
					title={'DeFI'} />
			</div>
		</section>
	);
